(function () {
  ...
  // ── PDF 转换守卫：在所有页面渲染完成前禁用生成按钮 ──────────────────
  var _origCanvasToBlob = HTMLCanvasElement.prototype.toBlob;
  var _pdfConvertDebounce = null;
  var _pdfConvertFallback = null;
  ...
  function findGenerateBtn() {
    return Array.from(document.querySelectorAll("button")).find(function (btn) {
      ...
      return /生成/.test(t) && !/上传|API|保存|恢复/.test(t);
    });
  }

  function setPdfConverting(on) {
    if (on) {
      var btn = findGenerateBtn();
      if (!btn) return;
      btn.setAttribute("data-yunwu-converting", "1");
      btn._yunwuOrigHTML = btn.innerHTML;
      btn.innerHTML = "<span>PDF 转换中…</span>";
      btn.disabled = true;
    } else {
      var guard = document.querySelector("[data-yunwu-converting]");
      if (!guard) return;
      if (guard._yunwuOrigHTML) guard.innerHTML = guard._yunwuOrigHTML;
      guard.disabled = false;
      guard.removeAttribute("data-yunwu-converting");
    }
  }

  function clearPdfConvertGuard() {
    clearTimeout(_pdfConvertDebounce);
    clearTimeout(_pdfConvertFallback);
    _pdfConvertDebounce = null;
    _pdfConvertFallback = null;
    HTMLCanvasElement.prototype.toBlob = _origCanvasToBlob;
    setPdfConverting(false);
  }

  function armCanvasToBlobMonitor() {
    HTMLCanvasElement.prototype.toBlob = function (cb, type, quality) {
      clearTimeout(_pdfConvertDebounce);
      var result = _origCanvasToBlob.call(this, cb, type, quality);
      _pdfConvertDebounce = setTimeout(clearPdfConvertGuard, 600);
      return result;
    };
    clearTimeout(_pdfConvertFallback);
    _pdfConvertFallback = setTimeout(clearPdfConvertGuard, 45000);
  }
  ...
  document.addEventListener(
    ...
    function (evt) {
      ...
      armCanvasToBlobMonitor();
      setTimeout(function () {
        if (_pdfConvertDebounce !== null || _pdfConvertFallback !== null) {
